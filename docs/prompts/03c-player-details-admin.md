A detailed profile page for a soccer player with a dark hero section and a clean white editorial section for stats and user comments. Specific for ADMIN users.
...
5. **Comments Section:**
   ...
   - **Comment List:** Cards with author name, star rating, comment text, and an "Added from [Location]" small badge. 
     - **Admin Feature:** Include inline "Delete" text buttons (#cf202f) next to each comment.
6. **Modals & Overlays:**
   - **Delete Confirmation Modal:** When "Delete" is tapped, show an alert dialog overlay with a "Cancel" button and a red "Delete" pill CTA.
   - **Add Comment Modal:** Opens when the "Add Comment" icon is tapped. A bottom sheet (24px radius) including a Textarea, 5-star rating selector, and a "Submit Note" pill CTA.