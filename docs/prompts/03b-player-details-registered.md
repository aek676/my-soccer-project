A detailed profile page for a soccer player with a dark hero section and a clean white editorial section for stats and user comments. Specific for REGISTERED users.
...
6. **Add Comment Modal (Registered Version):**
   - Opens when the "Add Comment" icon is tapped.
   - A bottom sheet or modal overlay with rounded top corners (24px radius).
   - Includes a Textarea, 5-star rating selector, and a "Submit Note" pill CTA. (The "Author Name" input is hidden, defaults to logged-in username).